...
public class MunSurveillanceCameraController : MonobitEngine.MonoBehaviour
{
    [SerializeField]
    ...
    private IEnumerator SetUp()
    {
        ...
        while (false == MonobitNetwork.room.customParameters.ContainsKey(IS_SURVEILLANCE))
        {
            yield return null;
        }
        var is_surveillance = (bool)MonobitNetwork.room.customParameters[IS_SURVEILLANCE];
    ...
    void OnMonobitCustomRoomParametersChanged(Hashtable peopertiesThatChanged)
    {
        ...
        if (false == peopertiesThatChanged.ContainsKey(IS_SURVEILLANCE))
        {
            return;
        }

        var is_debug_mode = (bool)peopertiesThatChanged[IS_SURVEILLANCE];
